    			...
    			vis.ready(function(){
    				...
    				vis.addListener("dblclick","nodes",function(evt){
    					...
    					if(node.data.type=="SNPcount")
    					{
    						...
    						{
    							if(network_json["data"]["edges"][temp]["target"]==node.data.id)
    							{
    								for(var temp2 in network_json["data"]["edges"])
    								{
    									if(network_json["data"]["edges"][temp2]["source"]==network_json["data"]["edges"][temp]["source"])
    									{
    										to_display.push(network_json["data"]["edges"][temp2]["target"]);		
    									}	
    								}
    							}
    						}	
